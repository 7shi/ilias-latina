...
`make text`, which writes the published text `../texts/iliad.txt`.
...
make text           # add the book headings (../texts/iliad.txt)
```
...
| `text` | `../texts/iliad.txt` — the same with `## N` book headings | `books.py` |
...
uv run python books.py tmp/ilias.txt ../texts/iliad.txt
...
`../texts/iliad.txt` is generated by `books.py`, so do not move its
